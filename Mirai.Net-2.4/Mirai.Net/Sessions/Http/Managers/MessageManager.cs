...
public static class MessageManager
{
    ...
    /// <param name="chain"></param>
    /// <returns></returns>
    public static async Task<string> SendGroupOrTempMessageAsync(string qq, string group, MessageChain chain)
    {
        var payload = new
        {
            target = group,
        ...
        string messageId = await SendMessageAsync(HttpEndpoints.SendGroupMessage, payload);
        if(messageId == "-1")
        {
            var new_payload = new
            {
            ...
            messageId = await SendMessageAsync(HttpEndpoints.SendTempMessage, new_payload);
        }
        return messageId;
    }
