...
public partial class MessageChain : List<MessageBase>
{
    ...
    {
        var at = this.OfType<AtMessage>().ToList();
        if (!at.Any())
            return string.Empty;

        return at.Select(x => x.Target).JoinToString("");
    }
    ...
    /// </summary>
    /// <returns>如果没有文本消息返回空字符串</returns>
    public string GetImageBase64Message()
    {
        var image = this.OfType<ImageMessage>().ToList();
        if (!image.Any())
            return string.Empty;

        return image.Select(x => x.Base64).JoinToString("#");
    }
